...
// 2) LEER MÁS (si llegara a aparecer de flujos viejos)
if (msg === "LEER_MAS") {
await sendLeerMas(from);
return res.sendStatus(200);
}

// 3) NUEVO MENÚ PRINCIPAL DESDE LA LISTA
if (msg === "MAIN_MENU") {
await sendMenuPrincipal(from);
return res.sendStatus(200);
}

if (msg === "MAIN_PRODUCTOS") {
...
// 4) MENÚ PRINCIPAL VIEJO (por compatibilidad)
...
// 7) PRODUCTO SELECCIONADO DIRECTO (por si en el futuro usás IDs tipo PROD_)
...
// 8) FOOD TRUCK (ids antiguos)
if (msg === "FOOD_TRUCK") {
await sendFoodTruck(from);
return res.sendStatus(200);
}

// 9) CONSULTAR PEDIDO (ids antiguos)
if (msg === "CONSULTAR_PEDIDO") {
await sendConsultarPedido(from);
return res.sendStatus(200);
}

// 10) CATÁLOGO PDF (id antiguo)
if (msg === "CATALOGO_PDF") {
await sendCatalogoCompleto(from);
return res.sendStatus(200);
}

// 11) CUALQUIER OTRA COSA → IA
