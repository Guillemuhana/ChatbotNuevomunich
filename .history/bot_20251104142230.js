...
// ====== Recepción de mensajes y manejo de flujo ======
export async function handleIncomingLogic(msg) {
const from = msg.from; // número del cliente
const type = msg.type;

// Si es texto normal -> respondemos con bienvenida si dice "hola"
if (type === "text") {
...
// Si está dentro de un flujo de pedido
return handleOrderFlow(from, text);
}

// Si es botón presionado (interactive reply)
...
default:
// Si selecciona categoría CATSEL_XXXXX
if (payload.startsWith("CATSEL_")) {
const catId = payload.replace("CATSEL_", "");
return sendCategoryItems(from, catId);
}

// Si es parte del pedido
